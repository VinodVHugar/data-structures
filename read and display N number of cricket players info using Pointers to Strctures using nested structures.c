//Program to read and display N number of cricket players info using Pointers to Strctures using nested structures.
#include<stdio.h>
struct info
{
int highest_score;
...
scanf ("%f",&p->average);
printf ( "\nEnter highest score");
scanf ("%d", &p->inf.highest_score );
printf ( "\nEnter Centuries ");
scanf ("%d", &p->inf.centuries);
printf ( "\nEnter ODI rank");
scanf ("%d", &p->inf.ODI_rank);
p++;
}
}
void display( PLR *p,int n)
...
printf ( "Team Name: %s\n",p[index].team_name);
printf ( "Average: %f\n",p[index].average);
printf ( "Highest score: %d\n",p[index].inf.highest_score);
printf ( "Centuries: %d\n",p[index].inf.centuries);
printf ( "ODI rank: %d\n",p[index].inf.ODI_rank);
}
void sort_by_ODI_rank(PLR *p, int n)
{
int i,j;
...
for(j=0;j<n-i-1;j++)
{
if(p[j].inf.ODI_rank>p[j+1].inf.ODI_rank)
{
swap=p[j];
p[j]=p[j+1];
p[j+1]=swap;
